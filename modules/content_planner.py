...
class ContentPlannerWidget(QWidget):
    ...
    def generate_plan(self, clips_info_list, start_date=None, posts_per_day=None, platforms=None, start_hour=None):
        ...
        for i, clip_info in enumerate(clips_info_list):
            ...
            if self.parent_window and hasattr(self.parent_window, 'subtitles_generator'):
                generated_hashtags = self.parent_window.subtitles_generator.generate_hashtags(description, top_n=3)
            else:
                # Логируем, что генератор хэштегов недоступен и пропускаем
                if self.parent_window:
                    self.parent_window.log_message(
                        "Контент-план: subtitles_generator не инициализирован, пропуск генерации хэштегов",
                        level="WARN"
                    )
                generated_hashtags = []
    ...
    def _highlight_calendar_dates_with_posts(self):
        ...
        # Попробуем цвет из вашей Nord-палитры для акцента
        highlight_format.setBackground(QBrush(QColor("#A3BE8C")))  # Nord Frost - зеленый (для "есть посты")
        highlight_format.setForeground(QBrush(QColor("#2E3440")))  # Темный текст для контраста
        highlight_format.setFontWeight(QFont.Weight.Bold)
        highlight_format.setToolTip("Есть запланированные посты")

        current_date_highlight_format = QTextCharFormat()
        current_date_highlight_format.setBackground(
            QBrush(QColor("#B48EAD")))  # Nord Aurora - фиолетовый (для "сегодня + посты")
        current_date_highlight_format.setForeground(QBrush(QColor("#ECEFF4")))
        current_date_highlight_format.setFontWeight(QFont.Weight.Bold)
        current_date_highlight_format.setToolTip("Сегодня есть запланированные посты")
        ...
        for q_date in planned_qdates:
            if q_date.isValid():
                if q_date == today:
                    self.calendar_widget.setDateTextFormat(q_date, current_date_highlight_format)
                else:
                    self.calendar_widget.setDateTextFormat(q_date, highlight_format)

        # Подсветка текущего дня, даже если нет постов (опционально)
        today_format = QTextCharFormat()
        today_format.setFontUnderline(True)
        today_format.setUnderlineColor(QColor("#D08770"))  # Nord Aurora - оранжевый
        if today not in planned_qdates:  # Если еще не подсвечен как день с постами
            self.calendar_widget.setDateTextFormat(today, today_format)
